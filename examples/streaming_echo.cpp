        ...
        while( true )
            try
            {
                ...
                try
                {
                    ...
                    // Always require a Content-Length header for this
                    // application
                    ...
                    // Create stream objects for request & response, as well as
                    // a read buffer for the echoed contents
                    std::istream  request_stream( &request  );
                    std::ostream response_stream( &response );
                    char buffer[ buffer_size ];
                    
                    // Note that std::istream::eof() only returns true after the
                    // first read operation to fail because of EOF.  Note you
                    // could instead use `show::request::oef()`, but this is
                    // meant to demonstrate using the stdlib stream objects.
                    do
                    {
                        // Block until we can read at least one byte or we reach
                        // the end.  This is to prevent the server from going
                        // into a busy loop while waiting for content from the
                        // client.
                        buffer[ 0 ] = request_stream.get();
                        // `std::istream` & `std::ostream` swallow all
                        // exceptions and set `badbit`, so code that uses
                        // streams for reading & writing will never actually be
                        // able to catch a `show::client_disconnected` or
                        ...
                        // Read additional available bytes up to the end of the
                        // buffer
                        ...
                        // Most likely don't want to manually flush in
                        // production code, but this allows you see the
                        // streaming in action
                        response.flush();
